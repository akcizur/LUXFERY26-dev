...
function System({open}:{open:(a:AppId)=>void}){return <div className="app-fill control"><Menu items={["Soubor","Úpravy","Zobrazit","Nápověda"]}/><h3>Ovládací panely</h3><div className="cp">{["Zobrazení","Myš","Klávesnice","Datum a čas","Zvuky","Síť","Uživatelé","Systém","Napájení","Programy"].map(x=><button key={x}>{x}</button>)}</div><div className="sysinfo sunken"><b>LUXFERY 26</b><br/>Windows 98 style desktop runtime<br/>React + TypeScript + Vite<br/>Build 26.09.04</div><Btn onClick={()=>open("notepad")}>Otevřít Poznámkový blok</Btn></div>}

export function App(){
 const [wins,setWins]=useState<Win[]>([]),[active,setActive]=useState<number|null>(null),[start,setStart]=useState(false),[id,setId]=useState(1),[fs]=useState(initialFs);
 ...
 return <main className="desktop"><div className="wallpaper" onMouseDown={()=>setStart(false)}>
  <div className="desktop-icons">{apps.map(a=><button className="desktop-icon" key={a.app} onDoubleClick={()=>open(a.app)}><span>{a.icon}</span><b>{a.title}</b></button>)}</div>
 ...
 </div><div className="taskbar"><button className="start-button" onClick={e=>{e.stopPropagation();setStart(!start)}}>▰ <b>Start</b></button><div className="tasks">{wins.map(w=><button key={w.id} className={active===w.id&&!w.minimized?"task-active":""} onClick={()=>{update(w.id,{minimized:!w.minimized});setActive(w.id)}}>{apps.find(a=>a.app===w.app)?.icon} {w.title}</button>)}</div><div className="tray">🔊　▧　{clock.toLocaleTimeString("cs-CZ",{hour:"2-digit",minute:"2-digit"})}</div></div></main>
}