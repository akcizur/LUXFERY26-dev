...
const desktopDefaults: DesktopEntry[] = [
...
function loadSavedSession(): SessionState {
  const saved = loadSession<SessionState>();
  if (!saved || !Array.isArray(saved.windows)) return { windows: [], nextZ: 20 };
  const migrated = saved.windows.filter((item) => typeof item.id === "string" && typeof item.appId === "string").map((item) => ({
    ...item,
...
function Minesweeper() { const make = () => Array.from({ length: 81 }, (_, index) => ({ index, mine: [3, 8, 11, 22, 35, 46, 57, 61, 70, 76].includes(index), open: false, flag: false })); const [cells, setCells] = useState(make); const [status, setStatus] = useState("HRA"); const [time, setTime] = useState(0); useEffect(() => { if (status !== "HRA") return; const timer = window.setInterval(() => setTime((v) => v + 1), 1000); return () => window.clearInterval(timer); }, [status]); const click = (index: number) => { if (cells[index].flag || cells[index].open || status !== "HRA") return; const next = [...cells]; next[index] = { ...next[index], open: true }; setCells(next); if (next[index].mine) { setCells(next.map((cell) => ({ ...cell, open: cell.open || cell.mine }))); setStatus("PROHRA"); } else if (next.filter((cell) => !cell.mine && !cell.open).length === 0) setStatus("VÝHRA"); }; return <div className="mine"><div className="mine-head">💣 10 <button onClick={() => { setCells(make()); setStatus("HRA"); setTime(0); }}>🙂</button> ⏱ {time}</div><div className="mine-grid">{cells.map((cell) => <button key={cell.index} className={`mine-cell ${cell.open ? "open" : ""}`} onClick={() => click(cell.index)} onContextMenu={(event) => { event.preventDefault(); const next = [...cells]; next[cell.index] = { ...next[cell.index], flag: !next[cell.index].flag }; setCells(next); }}>{cell.open ? cell.mine ? "💣" : "" : cell.flag ? "🚩" : ""}</button>)}</div><div className="status">{status}</div></div>; }

function Terminal({ onLaunch }: { onLaunch: (id: string) => void }) { const [lines, setLines] = useState(["LUXFERY 26 Terminal [Version 26.09]", "Napište 'help' pro seznam příkazů."]); const [input, setInput] = useState(""); const execute = () => { const command = input.trim(); if (!command) return; const lower = command.toLowerCase(); const output = lower === "help" ? "help, dir, ver, cls, echo <text>, open <app>, about" : lower === "dir" ? apps.map((app) => app.id).join("  ") : lower === "ver" ? "LUXFERY 26 virtual OS" : lower === "about" ? "Browser OS shell / LUXFERY 26" : lower === "cls" ? "" : lower.startsWith("echo ") ? command.slice(5) : lower.startsWith("open ") ? (appMap.has(lower.slice(5)) ? (onLaunch(lower.slice(5)), "Spouštím…") : "Aplikace nebyla nalezena.") : "Neznámý příkaz."; setLines((previous) => lower === "cls" ? [] : [...previous, `C:\\LUXFERY>${command}`, output]); setInput(""); }; return <div className="terminal"><div className="terminal-output">{lines.map((line, index) => <div key={`${index}-${line}`}>{line}</div>)}</div><div className="terminal-input"><span>C:\\LUXFERY&gt;</span><input autoFocus value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && execute()} /></div></div>; }

function Settings({ settings, onChange }: { settings: DesktopSettings; onChange: (patch: Partial<DesktopSettings>) => void }) { return <div className="app-fill control"><MenuBar items={["Soubor", "Úpravy", "Zobrazit", "Nápověda"]} /><h3>Ovládací panely</h3><div className="settings-grid"><label>Téma<select value={settings.theme} onChange={(event) => onChange({ theme: event.target.value as DesktopSettings["theme"] })}><option value="classic">Classic</option><option value="dark">Dark</option><option value="high-contrast">High contrast</option></select></label><label>Poloha taskbaru<select value={settings.taskbarPosition} onChange={(event) => onChange({ taskbarPosition: event.target.value as DesktopSettings["taskbarPosition"] })}><option value="bottom">Dole</option><option value="top">Nahoře</option></select></label><label className="check"><input type="checkbox" checked={settings.reduceMotion} onChange={(event) => onChange({ reduceMotion: event.target.checked })} /> Omezit pohyb</label><label className="check"><input type="checkbox" checked={settings.soundEnabled} onChange={(event) => onChange({ soundEnabled: event.target.checked })} /> Systémové zvuky</label></div><p>Retro pozadí zůstává aktivní jako wallpaper engine LUXFERY.</p></div>; }

function ExternalApp({ app }: { app: AppDefinition }) { const [opened, setOpened] = useState(false); const open = () => { if (!app.url) return; window.open(app.url, "_blank", "noopener,noreferrer"); setOpened(true); }; useEffect(() => { if (!opened) open(); }, [opened]); return <div className="external-launcher"><div className="external-icon">{app.icon}</div><h3>{app.name}</h3><p>{app.description}</p><p className="external-url">{app.url}</p><Button onClick={open}>Otevřít web</Button><p className="external-note">Služba se otevře v novém panelu. Weby mohou zakázat vložení do iframe.</p></div>; }
