...
export function App(){
 ...
 return <main className={"desktop "+(settings.theme==="dark"?"theme-dark ":"")+(settings.theme==="high-contrast"?"theme-contrast ":"")+(settings.reduceMotion?"reduce-motion":"")}><div className="wallpaper" style={{background:settings.wallpaper}} onMouseDown={()=>setStart(false)}>
  <div className={"desktop-icons "+(settings.showSmallIcons?"small-icons":"")} style={{display:settings.showDesktopIcons?"grid":"none"}}><div className="desktop-scale" style={{transform:`scale(${scale})`}}>{apps.map(a=><button className="desktop-icon" key={a.app} onDoubleClick={()=>open(a.app)}><span>{a.icon}</span><b>{a.title}</b></button>)}</div></div>
