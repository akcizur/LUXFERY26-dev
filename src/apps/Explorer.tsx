...
type Props = { onLaunch: (id: string) => void };
...
export function Explorer({ onLaunch }: Props) {
  ...
  const open = (node: FsNode) => { if (node.type === "folder") setCurrentId(node.id); else if (node.ext?.toLowerCase() === ".txt") onLaunch("notepad"); };
  ...
  const createFolder = () => { const parent = findNode(fs, currentId); if (!parent || parent.type !== "folder") return; const name = uniqueName(parent, "Nová složka"); const next = updateTree(fs, currentId, (node) => ({ ...node, children: [...(node.children ?? []), { id: crypto.randomUUID(), name, type: "folder", children: [] }] })); commit(next); };
  const createText = () => { const parent = findNode(fs, currentId); if (!parent || parent.type !== "folder") return; const name = uniqueName(parent, "Nový dokument.txt"); const next = updateTree(fs, currentId, (node) => ({ ...node, children: [...(node.children ?? []), { id: crypto.randomUUID(), name, type: "file", ext: ".txt", size: "0 KB" }] })); commit(next); };
  ...
  const remove = () => { if (!selected || selected.id === "root") return; const next = updateTree(fs, "recycle", (node) => ({ ...node, children: [...(node.children ?? []), { ...selected, id: `trash-${selected.id}-${Date.now()}`, name: selected.name, deleted: true }] })); commit(removeTree(next, selected.id)); setSelectedId(null); };
  const copy = () => selected && setClipboardId(selected.id);
  ...
  const emptyRecycle = () => { const next = updateTree(fs, "recycle", (node) => ({ ...node, children: [] })); commit(next); };
  const restoreSelected = () => { if (!selected || !selected.deleted) return; const recycle = findNode(fs, "recycle"); const desktop = findNode(fs, "desktop"); if (!recycle || !desktop) return; const next = updateTree(removeTree(fs, selected.id), "desktop", (node) => ({ ...node, children: [...(node.children ?? []), { ...selected, id: crypto.randomUUID(), deleted: false, name: uniqueName(node, selected.name) }] })); commit(next); setSelectedId(null); };
